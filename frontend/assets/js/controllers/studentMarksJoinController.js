...
import {
  ...
  downloadCSV,
  downloadPDF
} from "../components/MarkJoinTable.js";
...
/**
 * ROUTE INITIALIZER
 * Called EVERY time /student-marks-join is visited
 */
export async function initStudentMarksJoin() {
  console.log("Initializing Student Marks Join Page...");

  // 🔥 CRITICAL FIX: clear existing rows before rendering
  ...
  // 2. Render table (initial load = true)
  renderMarkJoinTable(rows, true);

  // 3. Initialize search & sort (safe to re-run)
  initReportSearchSort();

  // 4. Re-bind export buttons
  const csvBtn = document.getElementById("exportCSV");
  const pdfBtn = document.getElementById("exportPDF");
